			...
			for (Object h : hotelList) {
				...
				for (Object r : roomList) {
					...
					String cmd = "INSERT INTO RoomList"
							+ "(HotelID, HotelStar, StreetAddress, Locality, RoomType, RoomPrice, Number)" + "VALUES"
							+ "(" + hotel.get("HotelID").toString() + ", " + hotel.get("HotelStar").toString() + " , "
							+ "\"" + hotel.get("Street-Address").toString() + "\"" + ", " + "\""
							+ hotel.get("Locality").toString() + "\"" + ", " + "\"" + room.get("RoomType").toString()
							+ "\"" + ", " + room.get("RoomPrice").toString() + ", " + room.get("Number").toString()
							+ ");";
					try {
						stmt.execute(cmd);
					} catch (Exception e) {
